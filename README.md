...
``` r
library(dplyr)
citation_list = lapply(citations, parse_scholar_cites)
n = lapply(citation_list, nrow) %>% unlist()

cid = str_extract(citation_url2, "[0-9,]+$")
source = rep(cid, time = n)

citation_data = bind_rows(citation_list)
citation_data$source = source
...
``` r
data("zotero_json")
zotero_data = lapply(seq_along(zotero_json), function(i){
  d = parse_zotero_json(zotero_json[[i]])
  d$fulltextUrl = unique_fulltextUrl[[i]]
  return(d)
}) %>% bind_rows() %>%
...
``` r
citation_data %>% left_join(zotero_data %>% select(-title)) %>% filter(is.na(publicationTitle))
```
